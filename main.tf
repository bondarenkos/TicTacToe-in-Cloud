...
resource "aws_ecs_task_definition" "tictactoe_backend_task" {
  ...
  cpu                      = "512"
  memory                   = "1024"
  execution_role_arn       = "arn:aws:iam::211125483624:role/LabRole"
  task_role_arn            = "arn:aws:iam::211125483624:role/LabRole"

  container_definitions = jsonencode([
    {
      name        = "backend"
      image       = "211125483624.dkr.ecr.us-east-1.amazonaws.com/tictactoe_back:latest"
      essential   = true
      portMappings = [
        {
          containerPort = 8080
...
resource "aws_ecs_task_definition" "tictactoe_frontend_task" {
  ...
  container_definitions = jsonencode([
    {
      name        = "frontend"
      image       = "211125483624.dkr.ecr.us-east-1.amazonaws.com/tictactoe_front:latest"
      essential   = true
      portMappings = [
        {
          containerPort = 3000
